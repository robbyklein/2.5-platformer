...
void APlatformerCharacter::BeginPlay()
{
	...
	// Create a camera
	ACameraManager* CameraManager = GetWorld()->SpawnActor<ACameraManager>(GetActorLocation(), GetActorRotation());

	if (CameraManager)
	{
		CameraManager->SetFollowTarget(this);

		// If we have a player controller, set the new camera as active camera
		if (APlayerController* PC = Cast<APlayerController>(GetController()))
		{
			PC->SetViewTargetWithBlend(CameraManager, 0.5f);
		}
	}
...
void APlatformerCharacter::Landed(const FHitResult& Hit)
{
	...
	// Check if the actor the character landed on has a ULandKill component
	if (AActor* LandedOnActor = Hit.GetActor())
	{
		if (UKillable* KillableComponent = LandedOnActor->FindComponentByClass<UKillable>())
		{
			// If it does, destroy the actor
			UE_LOG(LogTemp, Display, TEXT("Landed!"));
			LandedOnActor->Destroy();
		}
	}
}
